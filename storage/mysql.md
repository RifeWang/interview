...
最大连接数由 `max_connections` 配置（默认 151），每条连接分配一个独立的线程。
...
MySQL 5.7 及以上版本，InnoDB 默认 page 为 16 KB，对于 B+ Tree，每个节点都是一个 page，根节点 16 KB，假设索引中的主键使用自增 BIGINT 8 B、页号 4 B，那么那么每个 page 可以存储 15 KB/12 B = 1280 条索引。

实际树的高度取决于每行数据占用的大小：
    - 假设 page 为 16 KB，可通过 `innodb_page_size` 设置
    - 假设 page 中存放数据的大小是 15 KB
    - 如果每行数据大小为 1 KB，则树为 1 层时，只能存储 15 行数据
    - 假设索引页的一条数据为 12 B，那么每个 page 可以存储 15 KB/12 B = 1280 条索引
        - 树为 2 层时，可以存储 1280 * 15 = 19200 行数据
        - 树为 3 层时，可以存储 1280^2 * 15 = 24576000 （约 2.45 kw），经验上建议单笔 2 千万行就是这么来的
        - 数为 4 层时，可以存储 1280^3 * 15 = 31457280000（约 314 亿），所以 B+ Tree 的高度为 3 ~ 4 层。


page 大小：
- I/O 是以 page 为单位，即使只需要页面中的一小部分数据，也必须读取整个页面。
- 大 page 的优点：
    - 可以存储更多行数据，减少 page 分裂和合并的频率，可能提高顺序 I/O 的性能
- 大 page 的缺点：
    - 降低随机 I/O 效率（读取大量不必要的数据占用带宽）
    - 降低缓存效率
    - 多个事务访问同一个大页还会导致并发资源竞争
    - 写入放大。即使只修改小部分，也要写回整个页面
- OLTP & OLAP：
    - OLTP 通常受益于较小的页面大小，因为它们常涉及小型随机访问。
    - OLAP 可能更适合大页面，因为它们通常进行大规模顺序扫描。
...
- 配置：
    ...
    - MySQL配置
- 表结构设计：
    - 选择合理的、大小合适的数据类型，尽量避免使用 BLOB 和 TEXT
    - 默认值尽量避免使用 NULL
    - 尽量避免使用外键和触发器
    - 构建合适的索引：三星索引
- 使用：
...
- 《高性能 MySQL 第四版》
- https://www.cnblogs.com/apachecn/p/18083123
