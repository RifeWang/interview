...
Q: 哈希算法为什么使用 `CRC16` 算法？
A: `CRC16` 算法计算速度快（对数据进行位运算，产生一个 16 bit 的校验值）；碰撞率低，数据分布会更均匀。

Q: 为什么是 `16384` 个哈希槽？
A: 16384 是一个 2 的幂次方（2^14），哈希值的计算和槽的映射可以通过位运算快速完成，减少计算开销。`slot = hash_value % 16384` 等价于 `slot = hash_value & (16384 - 1)`，将 hash_value 的低 14 位保留，其他位数清零，得到的结果就是取模的结果。

Q: 为什么不使用一致性哈希算法？
A: 一致性哈希算法的计算和实现更加复杂。哈希槽可以更加灵活地控制数据迁移。

Q: Redis 集群可以无限扩展吗？
A: 否。官方建议的上限是 1000 个节点。节点数量增加会导致集群管理的开销增大，比如 Gossip 广播消息的消耗。

Q: Redis 集群能够支撑 百万 QPS 吗？
A: 基于单个 Redis 节点的性能基准和 Redis Cluster 的线性扩展性理论，Redis Cluster 确实可以支撑百万级 QPS，甚至千万 QPS。然而实际性能会因硬件配置、网络条件、工作负载特性和具体的使用场景而有很大差异。[AWS 扩展到了单集群 5 亿 QPS 且响应时间为微秒](https://aws.amazon.com/blogs/database/achieve-over-500-million-requests-per-second-per-cluster-with-amazon-elasticache-for-redis-7-1/)
